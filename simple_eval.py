...
from simple_train import SimpleModel, VOCAB, VOCAB_SIZE, MAX_LENGTH, tokenize_simple
...
def generate_code(model, image):
    """Generate code from image using the model."""
    model.eval()
    with torch.no_grad():
        if image.dim() == 3:
...
def create_test_images(num_samples=10):
    """Create test images - in practice these would be real 3D renders."""
    images = []
    gt_codes = []
    ...
    for i in range(num_samples):
        ...
        # Ground truth code (in practice, would come from dataset)
        gt_codes.append('result = cq.Workplane("XY").box(1, 1, 1)')
    
    return images, gt_codes

def evaluate_simple_model(model_path: str, num_samples: int = 10):
    """Evaluate the simple model using our metrics."""
    ...
    for i, (image, gt_code) in enumerate(tqdm(zip(images, gt_codes))):
        ...
        # Convert simplified tokens back to proper CadQuery syntax
...
def main():
    ...
    vsr, iou = evaluate_simple_model(args.model_path, args.samples)
    print(f"Simple Model Results - VSR: {vsr:.3f}, IOU: {iou:.3f}")
